...
一方で、誤作成や運用上のノイズとして未完了のまま消したいCaseは存在しうる。この用途に限り、目立たない導線からCase削除を許可する。

削除時の扱い:

- System Caseは削除不可。
- 通常の「完了」操作では削除せず、Closed + Archiveを使う。
- 関連Taskは `deleted_at` による論理削除とする。
- 関連メール・Stakeholder・Tool Link・File Linkなどのリンクは解除または削除済み扱いにする。
- Case専用Storage Directoryは通常ディレクトリへ戻すなど、物理ファイルを失わない扱いにする。
- 監査ログは失わない。必要に応じてCase参照をNULL化してもよいが、操作履歴そのものは残す。
...
9. Taskは実削除せず、論理削除する。
10. Caseは原則削除しない。完了CaseはArchiveし、誤作成などの例外のみ確認付きDeleteを許可する。
